    ...
    public class RequisitoClaimFilter : IAuthorizationFilter
    {
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            if (!context.HttpContext.User.Identity.IsAuthenticated)
            {
                ...
                //context.Result = new RedirectToRouteResult(new RouteValueDictionary(new { controller = ".", action = "entrar", ReturnUrl = context.HttpContext.Request.Path.ToString() }));
                //context.Result = new RedirectToRouteResult(new RouteValueDictionary(new { controller = ".", page = "Login", ReturnUrl = context.HttpContext.Request.Path.ToString() }));
                return;
            }
