...
namespace DevIONETEC.Data.Repository
{
    ...
    {
        public PedidoItemsRepository(IonetecDbContext context) : base(context) { }
        ...
        //public async Task<PedidoItem> ObterPedidoItemsPeloPedido1(Guid pedidoId)
        //{
        //    return await Db.PedidoItems.AsNoTracking()
        //        .Include(p => p.ProdutoId)
        //       .FirstOrDefaultAsync(pi => pi.Id == pedidoId);
        //}
        public async Task<IEnumerable<PedidoItem>> ObterPedidoItemsPorProduto(Guid produtoId)
         ...
         public async Task<IEnumerable<PedidoItem>> ObterPedidoItemsPorVendedor(Guid vendedorId)
        {
            return await Buscar(pi => pi.VendedorId == vendedorId);
        }
        ...
        public async Task<PedidoItem> ObterPedidoItemVendedor(Guid id)
         {
            return await Db.PedidoItems.AsNoTracking().Include(v => v.Vendedor)
                .FirstOrDefaultAsync(pi => pi.Id == id);
         }
        ...
        public async Task<IEnumerable<PedidoItem>> ObterPedidosItemsProdutos()
         ...
         public async Task<IEnumerable<PedidoItem>> ObterPedidosItemsVendedores()
         {
            return await Db.PedidoItems.AsNoTracking().Include(v => v.Vendedor)
                 .OrderBy(pi => pi.Produto.Nome).ToListAsync();
         }
